...
>
> Este documento **no duplica** `docs/especificaciones/paginas-creacion-configuracion.md`
> (especificación maestra de páginas, formularios y campos):
> lo **referencia** y profundiza en lo que él no cubre — la **arquitectura de
...
- El % de IVA **no se teclea por producto**: se deriva de `clase_fiscal × territorio_fiscal`
  ...
  no el número en la ficha (ver `docs/especificaciones/17` §5.3 y carpeta `08-fiscalidad`).
...
> El detalle campo-a-campo de cada formulario está en `docs/especificaciones/17`; aquí se mapean **ámbitos** y
...
Implementado en comandera (`validar_pin`); pendiente en TPV (ver `docs/especificaciones/17` §9.4).
