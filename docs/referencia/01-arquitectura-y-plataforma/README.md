...
5. **Cumplir VERIFACTU/IGIC** con numeración y huella encadenada — escritura controlada, nunca «sucia» (ver **[07 — Facturación y cumplimiento legal](../../dossier/07-facturacion-y-cumplimiento-legal.md)**).
...
- **Tauri 2.0 es el ganador para escritorio**: reutiliza la UI de `apps/web` (React 19), pero el núcleo es **Rust**, con acceso directo a USB/serial/red para periféricos, binario de ~10 MB y auto‑update firmado. Es exactamente lo que ya anticipa **[04 — Arquitectura técnica](../../dossier/04-arquitectura-tecnica.md)** (`apps/desktop`).
...
- **Comandera → PWA por defecto, Expo si hace falta.** El 80 % de los locales imprimen en cocina por red (la comanda va a la impresora del KDS, no a una BT en el bolsillo). Para ese caso, una **PWA instalada en tablet Android en modo kiosco** es suficiente, más barata de mantener y comparte código con la web. Solo si el cliente exige impresora Bluetooth de mano, NFC para pagos o robustez nativa, se sube a **Expo** (`apps/mobile`, ver **[10](../../dossier/10-comanderas-kds-e-impresion.md)**).
...
2. **PowerSync sincroniza en ambos sentidos** por *buckets* segmentados por `tenant_id` (y `location_id`), como detalla **[06](../../dossier/06-base-de-datos-y-sincronizacion.md)**. Cada dispositivo solo descarga sus datos.
3. **El camino de escritura pasa por el backend.** Toda mutación sincronizada se valida en NestJS (`apps/api`): autorización por tenant, lógica fiscal y **numeración legal VERIFACTU**. Nada «sucio» entra en Postgres — principio ya fijado en **[04](../../dossier/04-arquitectura-tecnica.md)**.
...
2. **El envío a la AEAT se difiere.** VERIFACTU **no exige enviar en el segundo**: el registro se **encola** y la API NestJS lo remite a la AEAT **al recuperar la conexión**. El cliente ya tiene su ticket válido; Hacienda recibe el lote después. (Si el corte se alarga, el modo *VERI*FACTU lo contempla; el detalle legal, en **[07 — Facturación y cumplimiento legal](../../dossier/07-facturacion-y-cumplimiento-legal.md)**.)
...
La plataforma no cambia el modelo de datos: **shared schema + `tenant_id` + RLS**, exactamente como fija **[06](../../dossier/06-base-de-datos-y-sincronizacion.md)**. Lo que la arquitectura de plataforma añade es **cómo se proyecta ese aislamiento en cada dispositivo**:
...
*Documento de auditoría · 01 — Arquitectura y plataforma. Relacionados: [04](../../dossier/04-arquitectura-tecnica.md) · [05](../../dossier/05-stack-tecnologico.md) · [06](../../dossier/06-base-de-datos-y-sincronizacion.md) · [09](../../dossier/09-hardware.md) · [10](../../dossier/10-comanderas-kds-e-impresion.md).*
