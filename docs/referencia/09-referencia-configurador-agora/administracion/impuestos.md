...
> Tabla de **tipos impositivos** configurables: nombre, **valor %**, **recargo de equivalencia %**, **tipo** (IVA/IGIC/IPSI) y habilitado. Es la pieza que en Gluuh ya resuelve `@gluuh/core` (`tax-rates.ts`, `ivaAuto`) y la tabla SQL `tax_rate`/`resolver_iva()`. **Innegociable que cuadre** con el motor fiscal y el vector AEAT. Ver [03 — Catálogo](../../03-catalogo-productos-y-modificadores/) y [07-facturación](../../../dossier/07-facturacion-y-cumplimiento-legal.md).
