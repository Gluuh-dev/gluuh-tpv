...
| F5 (3 entregas) | LAN/media/IPC, soporte, backups | **5.1/5.2 núcleo ESCRITO · 5.3 soporte/break-glass APLICADO 17-07** · IPC=4.2 bloqueada · backups TODO | Gateway/media/descargador endurecidos (ver traspaso §13). **0121 aplicada**: `sesion_soporte` con consentimiento del titular (PENDIENTE→APROBADA), break-glass con MFA aal2 + motivo, **máximo 2 h por CHECK** (humo verde), sin autorrenovación, todo auditado en `evento_seguridad`; `soporte_vigente()` lista para el enforcement de Gestión remota. Backups cifrados (kit del titular) TODO |
