...
| quién | qué | ficheros |
|---|---|---|
| Claude (chat) | **SPA TPV (19-07): contraste del Inicio ARREGLADO + Configuración con mapa real — HECHO.** Contraste: `--color-muted` estaba **redefinido dos veces** en `@theme inline` (la 2ª, de `7d0afd7`, lo convertía en lavado al 10%) → los 63 `text-muted` de la SPA casi invisibles en claro Y oscuro. Quitada la redefinición (aviso en el propio `index.css`); el único `bg-muted` (badge P: del Ticket) pasa a `bg-surface-muted`. ⚠ El fix viajó en el commit `2f135a3` del escritorio (working tree compartido). **Configuración (F2)**: `config/secciones.tsx` = inventario del panel Next destilado en **6 dominios × 20 secciones** (carta / precios / salas y mesas / cobro / impresión / este terminal), cada una con su alcance y el origen del port en comentario; `Configuracion.tsx` = rail táctil + ficha de alcance honesta («se construye por fases; hoy, panel web»); **«Preferencias» ya funcional** (tema claro/oscuro del terminal). Reparto del hub: empleados/perfiles/licencias → apartado Administrador; zona técnica (VERIFACTU/copias/clave) → Visor Node. Build+typecheck verdes. | `apps/tpv/src/{index.css, apartados/config/*, apartados/inicio/Inicio.tsx, apartados/tpv/venta/Ticket.tsx}` |
