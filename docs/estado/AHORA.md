...
> **Actualizado:** 20-07-2026 · rama `nodo-local`
...
| quién | qué | ficheros |
|---|---|---|
| Claude (chat) | **ANÁLISIS: de 6 pantallas sueltas a un cuadro de mando con informes (20-07).** Se añaden **Diario de tickets** (ticket a ticket, lo que más pide un dueño), **Márgenes** (con menú engineering), **Alérgenos** y las tablas de **Asistencia**, **Familias**, **Formas de pago** y **No-ventas**. Catálogo de informes estilo Ágora: **15 de 41 disponibles**, y los que no se pueden servir salen atenuados **diciendo qué les falta** (enseñar 41 que abren pantallas vacías destruye la confianza en los 15 buenos). Filtro por **rango de fechas libre** además de los cuatro atajos, y descarga **CSV o PDF** desde cada tabla. **Nuevo componente `ui/Tabla.tsx`**: cabecera con buscador+contador+descarga, orden por columna, paginación y fila de totales; la columna se declara **UNA vez** y de ahí salen celda, orden, búsqueda, total y exportación — antes se declaraba dos veces (array para el CSV y `<td>` en el JSX) y eso no se ve: sale un PDF al que le falta una columna y nadie se entera. Se van ~330 líneas de `<table>` a mano y `BarraInforme` entero. **Gráficas propias en SVG/CSS** (`ui/graficas.tsx`): barras con eje, área, donut y dispersión — **sin librería** (un paquete de charts son cientos de kB en un mini-PC). ⚠ **Tres `falta` del catálogo MENTÍAN**: el escandallo (`product_format.coste`, 0128), los alérgenos (`product.alergenos[]`, 0016, **110 productos rellenos**) y los fichajes (`shift`, 0001) llevaban tiempo en el esquema — antes de dar por imposible un informe, mirar la BD. **Bugs cazados por sus propios tests** (todos «no dan error», solo cifras mal): `toISOString()` fechaba los informes **un día antes** (UTC vs medianoche española); el diario **sumaba 115 € más** que los KPI (pesos sin normalizar → último ticket negativo recortado a 0); ordenar por importe comparaba **texto** (9,50 por encima de 1.486,30) porque la columna guardaba el número ya formateado; el eje cogía «el primer escalón que quepa» y dejaba un tercio del gráfico en blanco; y un turno de noche (20:00→02:30) daba **−17,5 h**. **170 tests verdes.** ⚠ **Todo con datos DEMO**: la forma es la que traerá la jornada del nodo, pero **no está cableado** (espera al emparejado F4, igual que la ficha de artículo). ⚠ El hueco de alérgenos («vacío» = sin declarar Y sin alérgenos, indistinguibles) está apuntado en la skill **gluuh-base-datos**. | `apps/tpv/src/apartados/analisis/*`, `apps/tpv/src/ui/{Tabla,graficas}.tsx`, `apps/tpv/src/lib/exportar.ts` |
