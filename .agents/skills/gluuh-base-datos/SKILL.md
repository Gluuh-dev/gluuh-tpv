...
1. **Lo que se aplica son las migraciones** de `supabase/migrations/*.sql`,
   numeradas secuencialmente (`0001`…; a 07-07-2026 la última es `0074`
   —login local por código+clave (`app_user.clave_hash` + `verificar_clave_operario`);
   `0073` `app_user.codigo` + semilla; `0071`/`0072` RLS con `operario_permite()`—,
   aplicadas). Nueva migración = siguiente número libre + nombre descriptivo en snake_case.
