...
- **`connector.ts`** — `GluuhConnector`: obtiene credenciales y **sube los cambios locales a nuestro backend** (que valida tenant/permisos/fiscalidad antes de persistir). Si la subida falla, reintenta (clave offline).
...
import { AppSchema, GluuhConnector } from "@gluuh/sync";

const db = new PowerSyncDatabase({ schema: AppSchema, database: { dbFilename: "gluuh.db" } });
await db.connect(new GluuhConnector({
  apiUrl: "https://api.gluuh.app",
