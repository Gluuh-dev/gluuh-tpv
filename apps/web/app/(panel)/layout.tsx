...
export default function PanelLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground">
      ...
      {/* Submenú al lado (w-64): cabecera con icono, secciones separadas por línea,
        ...
        <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-surface md:flex">
          ...
          <div className="flex-1 overflow-y-auto px-2.5 py-2.5">
            {activa.sections.map((sec, si) => (
              <div key={si} className={si > 0 ? "mt-2 border-t border-border-muted pt-2.5" : ""}>
                {sec.title && <div className="mb-1 px-2 text-[10.5px] font-semibold uppercase tracking-wider text-(--text-muted)">{sec.title}</div>}
                <div className="space-y-0.5">
                  {sec.items.map((i, ii) => {
                    if (!i.href) return (
                      <div key={ii} className="flex h-8 items-center justify-between rounded-md px-2 text-[13px] text-(--text-muted) opacity-50" title="Próximamente">
                        <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-border-strong" />{i.label}</span>
                        <span className="rounded bg-surface-muted px-1.5 py-0.5 text-[10px]">pronto</span>
                      </div>
                    );
                    const activo = pathname === i.href || pathname.startsWith(`${i.href}/`);
                    const cls = `group/it flex h-8 items-center gap-2 rounded-md px-2 text-[13px] transition-colors ${activo ? "bg-brand/15 font-medium text-foreground" : "text-(--text-secondary) hover:bg-surface-overlay hover:text-foreground"}`;
                    const dot = <span className={`h-1.5 w-1.5 shrink-0 rounded-full transition-colors ${activo ? "bg-brand" : "bg-border-strong group-hover/it:bg-(--text-muted)"}`} aria-hidden />;
                    return i.blank
                      ? <a key={ii} href={i.href} target="_blank" rel="noreferrer" className={cls}>{dot}<span className="flex-1 truncate">{i.label}</span><ExternalLink className="h-3.5 w-3.5 shrink-0 opacity-50" /></a>
                      : <Link key={ii} href={i.href} className={cls}>{dot}<span className="flex-1 truncate">{i.label}</span></Link>;
                  })}
                </div>
              </div>
            ))}
