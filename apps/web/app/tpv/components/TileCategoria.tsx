...
// MEMOIZADO (como TileProducto): props primitivas + onClick estable que recibe el
// `id`, para que una recarga del catálogo no reconstruya toda la fila de categorías.
...
export interface TileCategoriaProps {
  /** Id de la categoría: se lo pasa al onClick (que es estable). */
  id: string;
  nombre: string;
  color: string;
...
export const TileCategoria = memo(function TileCategoria({ id, nombre, color, seleccionada, Icono, foto, onClick }: TileCategoriaProps) {
  return (
