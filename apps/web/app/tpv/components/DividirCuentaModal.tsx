...
// Modal DIVIDIR CUENTA — flujo decidido en docs/plan/dividir-cuenta-flujo-ux.md.
// Es el CENTRO DE MANDO del reparto: los cobros se abren ENCIMA (CobrarModal, lo
// renderiza page.tsx) y al confirmar SE VUELVE aquí. Tres formas, mezclables:
...
//     la mesa (separar_cuenta); aquí solo se preparan y se pide el cobro.
//   · Por importe — separa importes sueltos del pendiente; pago parcial.
// PRESENTACIONAL: las partes persistidas (cuenta_parte) llegan por props y toda
// mutación va por callbacks a page.tsx (que escribe en BD). Cerrar NO borra nada.
...
/** Parte persistida en `cuenta_parte` (la trae page.tsx de la BD). */
...
export interface DividirCuentaModalProps {
  lineas: LineaTicket[];
  /** Total ACTUAL de la mesa (ya sin lo cobrado por artículos). */
  total: number;
  /** total − Σ partes de dinero cobradas. Lo que falta por pagar. */
  pendiente: number;
  partes: ParteVista[];
  comensales?: number;
  contexto?: string;
  /** Cobrar la parte `pos` (0-based) de un reparto en `n` iguales del pendiente.
   *  page.tsx materializa el reparto en BD si hace falta y abre el cobro. */
  onCobrarIgual(n: number, pos: number): void;
  /** Separa un importe suelto del pendiente (persiste una parte IMPORTE). */
  onSepararImporte(importe: number): void;
  /** Cobrar una parte ya persistida (abre el cobro encima con su importe). */
  onCobrarParte(p: ParteVista): void;
  /** Separar Y cobrar el resto no repartido, de una. */
  onCobrarResto(importe: number): void;
  /** Cobrar por ARTÍCULOS: las líneas de la cuenta activa (salen de la mesa). */
  onCobrarCuenta(lineas: { id: string; uds: number }[]): void;
  /** Botón del pie: cobrar TODO el pendiente de una. */
  onCobrarPendiente(): void;
  /** Borra las partes NO cobradas (lo cobrado jamás se toca). */
  onQuitarDivision(): void;
  /** Imprime el justificante (proforma) de una parte. */
  onImprimirParte(etiqueta: string, importe: number): void;
  /** Cierra la vista. NO borra el reparto (persiste y reaparece). */
  onCerrar(): void;
  onAbrirCajon?(): void;
}
...
export function DividirCuentaModal({
  ...
  // Si aparece un cobro parcial de dinero estando en "Por productos", salir de ahí
  // (la pestaña queda deshabilitada; ver hayParcialesDinero).
  useEffect(() => {
  ...
  // Con partes de DINERO ya cobradas, sacar artículos descuadraría la cuenta (esas
  // líneas están medio pagadas por las partes): "Por productos" se deshabilita.
  ...
  // Las partes llegan de la BD DESPUÉS de montar (carga async al abrir): cuando
  // aparece un reparto persistido, el stepper se alinea con él (no preview).
  useEffect(() => {
    if (igualesPend.length) setNLocal(igualesPend.length);
  }, [igualesPend.length]);
  // Tarjetas pendientes: las de BD si cuadran con el stepper; si no, PREVIEW del
  // reparto (no se escribe nada hasta que se cobra la primera — así abrir a mirar
  // no deja la mesa "dividida" sin querer).
  ...
  /* ══ Modo 2: por productos (staging local; el cobro lo remata page.tsx) ══ */
  ...
  // Las claves de líneas ya cobradas (salieron de la mesa) desaparecen de `lineas`:
  // se ignoran en asignaciones y totales (la cuenta se "vacía" sola tras el cobro).
  ...
  // Resto = pendiente − todo lo ya repartido en partes de dinero sin cobrar.
    ...
    return (
      <div className={`flex flex-col gap-2.5 rounded-lg border p-3 ${cobrada ? "border-success/40 bg-success/10" : "border-border bg-background"}`}>
        ...
        <div className="flex gap-1.5">
          <button type="button" onClick={() => onImprimirParte(titulo, importe)} className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-md border border-border bg-card text-xs font-bold transition-all hover:bg-accent active:scale-[.98]">
          ...
          {cobrada ? (
            ...
            <button type="button" onClick={onCobrar} className="min-h-11 flex-1 rounded-md border border-warning bg-card text-xs font-bold text-warning transition-all hover:bg-warning/10 active:scale-[.98]">
              Cobrar
            </button>
          )}
        </div>
  ...
  return (
    ...
    >
        ...
        {([
          ...
          return (
            <button
              ...
              className={`flex min-h-12 items-center gap-2 rounded-md border px-4 text-sm font-semibold transition-all active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-40 disabled:active:scale-100 ${
            ...
            <div className="flex min-h-0 flex-col rounded-lg border border-border bg-card">
              ...
              <div className="flex flex-none gap-1.5 overflow-x-auto px-3 pb-2">
                {cuentas.map((c, i) => {
                  const on = i === ctAct;
                  return (
                    <button key={`ct-${i}`} type="button" onClick={() => setCtAct(i)} className={`flex min-h-13 flex-none flex-col justify-center gap-0.5 rounded-md border px-3.5 py-1.5 text-left transition-all ${on ? "border-brand bg-brand text-white" : "border-border bg-background hover:bg-accent"}`}>
              ...
              <div className="flex flex-none items-center gap-2 border-t border-border p-2.5">
                ...
                <button type="button" onClick={vaciarCuenta} disabled={!entradasVivas(cuentas[ctAct] ?? {}).length} className="min-h-11 rounded-md border border-border bg-card px-3 text-sm font-semibold transition-all hover:bg-accent active:scale-[.98] disabled:opacity-40">Devolver todo</button>
        ...
        {modo === "imp" && (
          <div className="grid h-full min-h-0 grid-cols-[340px_1fr] gap-3 max-[1150px]:grid-cols-[284px_1fr]">
            <div className="flex min-h-0 flex-col gap-2 rounded-lg border border-border bg-card p-3">
              ...
              <button type="button" onClick={separarImporte} disabled={bufValor <= 0 || bufValor > restoImp} className="min-h-14 flex-none rounded-md bg-brand text-base font-bold text-white transition-all active:scale-[.98] disabled:bg-surface disabled:text-muted-foreground">Separar este importe</button>
            </div>
              ...
              <div className="grid min-h-0 flex-1 auto-rows-min grid-cols-[repeat(auto-fill,minmax(196px,1fr))] content-start gap-2 overflow-y-auto p-3 max-[1150px]:grid-cols-[repeat(auto-fill,minmax(170px,1fr))]">
                ...
                {restoImp > 0 && (
                  <div className="flex flex-col gap-2.5 rounded-lg border border-dashed border-border bg-background p-3">
                    ...
                    <button type="button" onClick={() => onCobrarResto(restoImp)} className="min-h-11 rounded-md border border-warning bg-card text-xs font-bold text-warning transition-all hover:bg-warning/10 active:scale-[.98]">Cobrar el resto</button>
                  </div>
                )}
              </div>
            </div>
      ...
      <footer className="flex flex-none flex-wrap items-center gap-2.5 border-t border-border bg-surface px-4 py-3">
        <button type="button" onClick={onCerrar} className="flex min-h-13 items-center gap-2 rounded-md border border-border bg-card px-4 text-sm font-semibold text-muted-foreground transition-all hover:bg-accent active:scale-[.98]" title="Cierra la vista; el reparto se conserva">
          <X size={18} /> Cerrar
        </button>
        ...
        {pendientesBD.length > 0 && (
          <button
            ...
            className={`min-h-13 rounded-md border px-4 text-sm font-semibold transition-all active:scale-[.98] ${confQuitar ? "border-danger bg-danger/10 text-danger" : "border-border bg-card text-muted-foreground hover:bg-accent"}`}
