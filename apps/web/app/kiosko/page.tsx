...
export default function Kiosko() {
  ...
  /* ---- INICIO: pantalla de bienvenida con color de marca ---- */
      ...
      <div>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-5">
          <button
            ...
            className="flex h-48 w-48 flex-col items-center justify-center gap-3 rounded-xl bg-background shadow-sm"
            style={{ color: c }}
          >
            <Utensils className="h-14 w-14" />
            <span className="text-2xl font-bold">Comer aquí</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-2xl font-bold">Para llevar</span>
          </button>
        </div>
      </div>
    </div>
  ...
  if (paso === "ok") return (
    <div className="grid min-h-screen place-items-center bg-background p-6 text-center text-foreground">
      <div>
        <div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-brand text-brand-foreground">
          <Check className="h-12 w-12" />
        </div>
        <h1 className="mt-5 text-3xl font-bold">¡Pedido confirmado!</h1>
        <p className="mt-2 text-lg text-muted-foreground">Tu número de pedido</p>
        <div className="mt-2 text-8xl font-black tabular-nums leading-none">A-{numero}</div>
        <p className="mt-3 text-muted-foreground">Recógelo cuando aparezca en pantalla.</p>
        <button
          onClick={reiniciar}
          className="mt-8 rounded-md bg-brand px-8 py-3 font-semibold text-brand-foreground"
        >
          Nuevo pedido
        </button>
      </div>
  ...
  if (paso === "pago") return (
    <div className="grid min-h-screen place-items-center bg-background p-6 text-center text-foreground">
      <div className="w-full max-w-sm">
        ...
        <div className="my-3 text-6xl font-black tabular-nums">{eur(total)}</div>
        <p className="text-muted-foreground">{unidades} producto(s) · {tipoConsumo === "LOCAL" ? "Comer aquí" : "Para llevar"}</p>
        <p className="mb-6 mt-1 text-xs text-muted-foreground">(Pago simulado · integración real en docs/08)</p>
        <div className="grid gap-3">
          {["💳 Tarjeta", "📱 Bizum", "💶 Efectivo"].map((m) => (
            <button
              ...
              onClick={pagar}
              disabled={busy}
              className="rounded-md bg-brand py-4 text-lg font-bold text-brand-foreground disabled:opacity-50"
            >
              {busy ? "Procesando…" : m}
            </button>
          ))}
        </div>
        <button onClick={() => setPaso("carta")} className="mt-5 text-muted-foreground">
          ← Volver
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <div className="grid flex-1 grid-cols-[150px_1fr_320px]">
        ...
        <nav className="flex flex-col overflow-y-auto border-r border-border bg-card">
          {cats.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setCatSel(cat.id)}
              className="border-b border-border px-3 py-4 text-left text-sm"
              style={catSel === cat.id ? { background: c, color: fg, fontWeight: 600 } : undefined}
            >
              {cat.nombre}
            </button>
          ))}
        ...
        <section
          className="grid content-start gap-3 overflow-y-auto p-4"
        ...
        >
          {productos.map((p) => (
            ...
            >
              ...
              <div className="font-bold tabular-nums" style={{ color: c }}>{eur(p.precio)}</div>
              {carrito[p.id] ? (
                <span className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-md bg-brand text-sm font-bold text-brand-foreground">
                  {carrito[p.id]}
                </span>
              ) : null}
            </button>
        ...
        {/* Panel de pedido */}
        <aside className="flex flex-col border-l border-border bg-card p-4">
          <h2 className="mb-2 font-semibold">Tu pedido</h2>
          <div className="flex-1 overflow-y-auto">
            {unidades === 0 && <p className="text-muted-foreground">Toca para añadir.</p>}
            {Object.entries(carrito).map(([id, q]) => {
              const p = prods.find((x) => x.id === id)!;
              return (
                <div key={id} className="flex items-center gap-2 py-1.5 text-sm">
                  <span className="flex-1">{p.nombre}</span>
                  <button onClick={() => sub(id)} className="grid h-7 w-7 place-items-center rounded-md bg-muted">
                    <Minus className="h-3 w-3" />
                  </button>
                  <span className="w-5 text-center tabular-nums">{q}</span>
                  <button onClick={() => add(id)} className="grid h-7 w-7 place-items-center rounded-md bg-muted">
          ...
          <div className="mt-2 flex justify-between border-t border-border pt-2 text-xl font-bold tabular-nums">
            <span>TOTAL</span><span>{eur(total)}</span>
          </div>
          <button
            onClick={() => setPaso("pago")}
            disabled={!unidades}
            className="mt-3 flex items-center justify-center gap-2 rounded-md py-4 text-lg font-bold disabled:opacity-40"
            style={{ background: c, color: fg }}
          >
            Pagar <ArrowRight className="h-5 w-5" />
          </button>
        </aside>
      </div>
    </div>
