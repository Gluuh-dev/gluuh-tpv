...
export interface ColumnaInforme<T> {
  titulo: string;
  /** Valor PLANO de la celda: es lo que se ordena, se busca y se exporta. */
  valor: (fila: T) => string | number | null;
  /** Alinear a la derecha (importes, cantidades). */
  derecha?: boolean;
}
...
export function descargarCSV<T>(nombre: string, columnas: ColumnaInforme<T>[], filas: T[]): void {
  ...
  const cuerpo = filas.map((f) => columnas.map((c) => escCsv(c.valor(f))).join(";")).join("\n");
  const csv = `﻿${cabecera}\n${cuerpo}`;
