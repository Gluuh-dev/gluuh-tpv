...
// icono BLANCO; sin él, la placa usa el tono por defecto.
export function CabeceraModal({
  Icono, titulo, subtitulo, derecha, onCerrar, tono = "marca", color,
}: Readonly<{
  Icono: LucideIcon; titulo: string; subtitulo?: string; derecha?: ReactNode;
  onCerrar?: () => void; tono?: "marca" | "suave"; color?: string;
}>) {
  ...
  return (
    <header className={`flex items-center gap-3 px-4 py-3 ${suave ? "border-b border-line text-paper" : "bg-brand text-white"}`}>
      <span className={`grid h-9 w-9 flex-none place-items-center rounded-md ${placa}`} style={color ? { background: color } : undefined}><Icono size={18} /></span>
