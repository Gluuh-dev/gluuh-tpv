...
export function CabeceraModal({
  ...
  return (
    // `data-arrastrar`: es la BARRA DE TÍTULO — por aquí se coge la ventana para
    // moverla (ver Modal.tsx). `cursor-move` lo anuncia; `select-none` evita que
    // arrastrar seleccione el texto del título.
    <header data-arrastrar
      className={`flex cursor-move touch-none items-center gap-3 px-4 py-3 select-none ${suave ? "border-b border-line text-paper" : "bg-brand text-white"}`}>
      {onIcono
