...
export function BotonProducto({
  ...
  return (
    <button
      ...
      className={`relative flex min-h-[78px] flex-col justify-between overflow-hidden rounded-[9px] p-2 text-left shadow-[0_2px_0_rgba(0,0,0,.28)] ${
    ...
    >
      ...
      <span className="relative text-[11px] font-bold uppercase leading-tight [text-shadow:0_1px_2px_rgba(0,0,0,.45)]">{nombre}</span>
      <span className="relative text-sm font-semibold tabular-nums [text-shadow:0_1px_2px_rgba(0,0,0,.45)]">{eur(precio)}</span>
    </button>
  );
}
