    ...
    <Modal onCerrar={onCerrar} ancho="5xl" className="overflow-hidden p-0">
      ...
      <div className="flex h-[80vh] max-h-[820px] min-h-0 flex-col bg-background text-foreground">
        ...
        <footer className="flex flex-none flex-wrap items-center gap-2.5 border-t border-border bg-card px-3 py-2.5">
          <button type="button" onClick={onCerrar} className="flex min-h-13 flex-col items-center justify-center gap-0.5 px-4 text-xs font-semibold text-danger transition-colors active:scale-95"><XCircle size={24} /> Cancelar</button>
          <button type="button" onClick={onDividir} className="flex min-h-13 items-center gap-2 rounded-md border border-border bg-card px-4 text-sm font-semibold transition-all active:scale-[.98]"><Split size={16} /> Dividir</button>
          <span className="flex-1" />
          <button type="button" onClick={onCerrar} className="flex min-h-13 items-center gap-2 rounded-md border border-warning bg-card px-4 text-sm font-bold text-warning transition-all active:scale-[.98]"><Mail size={16} /> Enviar por email</button>
          <button type="button" onClick={onCerrar} className="min-h-13 rounded-md border border-warning bg-card px-4 text-sm font-bold text-warning transition-all active:scale-[.98]">Imprimir cuenta (F10)</button>
          <button type="button" onClick={cobrar} disabled={!puedeCobrar} className="min-h-13 rounded-md border border-warning bg-card px-4 text-sm font-bold text-warning transition-all active:scale-[.98] disabled:border-border disabled:bg-surface disabled:text-muted-foreground">Cobrar e imprimir (F11)</button>
          <button type="button" onClick={cobrar} disabled={!puedeCobrar} className="min-h-13 rounded-md bg-warning px-8 text-[17px] font-bold text-white shadow-md transition-all active:scale-[.98] disabled:bg-surface disabled:text-muted-foreground disabled:shadow-none">Cobrar (F12)</button>
        </footer>
      </div>
    </Modal>
  );
